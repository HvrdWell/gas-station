...
struct PhoneNumEntryView: View {
    ...
    let phoneNumMaskStr = "(XXX) XXX-XX-XX"
    ...
    var countryCodePicker: some View {
        ScrollViewReader { scrollView in
            ...
            .onAppear() {
                scrollView.scrollTo("RU", anchor: .top)
            }
...
struct PhoneNumEntryView_Previews: PreviewProvider {
    static var previews: some View {
        let viewModel = OnboardingEntryViewModel(viewRouter: ViewRouter())
        return PhoneNumEntryView(viewModel: viewModel)
    }
}
